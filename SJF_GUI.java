...
public class SJF_GUI extends JPanel {
    ...
    public SJF_GUI(Process[] processes,int n, double avgWaitingTime, double avgTurnaroundTime,int time) {
        this.n = n;
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (Process process : processes) {
            if (process.completed) {
                ...
                g.fillRect((process.wait + process.arrival) * 40 + 50, y, width, height);
                g.setColor(Color.BLACK);
                g.drawRect((process.wait+ process.arrival) * 40 + 50, y, width, height);
